...
fn part2(data: &str) -> usize {
    ...
    // existing cuboids into sub-cuboids that remain on.
    // If they intersect with an existing cuboid and are "on" then do the same as above, but also add
    // a new "on" cuboid that occupies the full volume of the latest one.
    let re = Regex::new(r"^(?P<op>on|off) x=(?P<x_lo>-?\d+)..(?P<x_hi>-?\d+),y=(?P<y_lo>-?\d+)..(?P<y_hi>-?\d+),z=(?P<z_lo>-?\d+)..(?P<z_hi>-?\d+)$").unwrap();
    ...
    for line in data.lines() {
        ...
        for (i, cuboid) in cuboids.iter().enumerate() {
                ...
                && z_hi >= cuboid.z_lo
            {
                ...
                // create new cuboids and append them to new_cuboids.
            }
        }
        for &index in remove_cuboids.iter().rev() {
            cuboids.remove(index);
        }
        if op == "on" {
            cuboids.push(Cuboid{ x_lo, x_hi, y_lo, y_hi, z_lo, z_hi })
        }
