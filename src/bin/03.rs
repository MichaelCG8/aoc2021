...
fn main() {
    ...
    let data = include_str!("../../inputs/03");
...
fn part1(data: &str) -> isize {
    ...
    let mut zeros: Vec<isize> = first_line
        .trim()
        .chars()
        .map(|c| match c {
            '0' => 1,
...
fn part2(data: &str) -> isize {
    ...
    for i in 0..line_length {
        let zeros = lines
            .iter()
            .filter(|line| line[i..i + 1] == '0'.to_string())
            .count();
        let ones = lines
            .iter()
            .filter(|line| line[i..i + 1] == '1'.to_string())
            .count();
        let most_common = if zeros > ones { '0' } else { '1' };
        lines = lines
            .iter()
            .filter(|line| line[i..i + 1] == most_common.to_string())
            .copied()
            .collect();
        if lines.len() == 1 {
            break;
